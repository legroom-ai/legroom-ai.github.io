# Legroom Rust build targets. `just` is not installed on dev boxes; this
...
help:
	@echo "Legroom Rust targets:"
	@echo "  make test               - cargo test --workspace"
	@echo "  make test-parity        - maturin develop + parity-run against fixtures"
	@echo "  make bench              - cargo bench --workspace"
	@echo "  make build-proxy        - release build + strip legroom-proxy, print size"
	@echo "  make build-wheel        - release wheel for legroom-py"
	@echo "  make verify-rust-core   - build + install + import-verify legroom._core"
...
test-parity:
	...
	$(MATURIN) develop -m crates/legroom-py/Cargo.toml
	$(CARGO) run -p legroom-parity -- run --fixtures $(FIXTURES)

bench:
	$(CARGO) bench --workspace

build-proxy:
	$(CARGO) build --release -p legroom-proxy
	@BIN=target/release/legroom-proxy; \
	if command -v strip >/dev/null 2>&1; then strip "$$BIN" || true; fi; \
	SIZE=$$(wc -c < "$$BIN"); \
	printf 'legroom-proxy: %s bytes (%.1f MiB)\n' "$$SIZE" "$$(echo "$$SIZE / 1048576" | bc -l)"

build-wheel:
	$(MATURIN) build --release -m crates/legroom-py/Cargo.toml
...
# mode (Finding #2 in LEGROOM_PROXY_LOG_FINDINGS_2026_05_03.md). The
...
# darwin wheel, legroom._core not built in test-extras + smoke-test,
...
# instantiate `SmartCrusher`, which hard-imports `legroom._core`.
ci-precheck-python:
...
build-e2e-wrap:
	docker build --platform linux/amd64 -f e2e/wrap/Dockerfile -t legroom-wrap-e2e .

run-e2e-wrap: build-e2e-wrap
	docker run --rm legroom-wrap-e2e
