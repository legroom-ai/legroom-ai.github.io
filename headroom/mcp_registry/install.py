"""Top-level orchestration: register Legroom MCP across detected agents."""
...
def build_legroom_spec(proxy_url: str = DEFAULT_PROXY_URL) -> ServerSpec:
    """Construct the canonical :class:`ServerSpec` for the legroom server.
    ...
    if proxy_url and proxy_url != DEFAULT_PROXY_URL:
        env["LEGROOM_PROXY_URL"] = proxy_url
    command = resolve_legroom_command()
    return ServerSpec(
        name="legroom",
        command=command[0],
...
def build_serena_spec(context: str) -> ServerSpec:
    ...
    startup. Legroom installs Serena by default, so without this flag every
...
def install_everywhere(
    ...
    *,
    ...
    """Install the legroom MCP server into every detected agent.
