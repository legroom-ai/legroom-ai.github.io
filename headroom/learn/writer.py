...
# Marker delimiters for Legroom-managed sections
_MARKER_START = "<!-- legroom:learn:start -->"
_MARKER_END = "<!-- legroom:learn:end -->"
...
def _build_section(recommendations: list[Recommendation]) -> str:
    ...
    lines = [
        ...
        f"*Auto-generated by `legroom learn` on {now} — do not edit manually*",
        "",
    ]
...
def extract_marker_block(file_content: str) -> str | None:
    """Return the raw text of the legroom:learn marker block, or None.
...
def _strip_marker_block(content: str) -> str:
    """Remove the legroom:learn marker block from text, tidying blank lines.
...
class ClaudeCodeWriter(ContextWriter):
    ...
    def write(
        ...
        if context_recs:
            ...
            # legroom versions into the new target, then strip it from CLAUDE.md
    ...
    def _migrate_legacy_block(
        ...
        """Move a stale legroom block out of CLAUDE.md into the new target.
        ...
        if cleaned:
            ...
            result.warnings.append(
                f"Moved Legroom learnings out of {legacy_path} into {target_path}: "
            ...
            # CLAUDE.md held nothing but the Legroom block — remove the husk.
            result.warnings.append(
                f"Removed {legacy_path} (it contained only Legroom learnings) and "
