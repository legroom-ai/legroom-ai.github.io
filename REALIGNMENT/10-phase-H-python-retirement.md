...
**Worktree:** `~/claude-projects/legroom-worktrees/realign-H1-retire-python-proxy-request-path`
**Risk:** **HIGH** (production-facing change; canary deploy mandatory)
**LOC:** **-15,000 / +500**

### Scope
Delete the Python FastAPI server, all handlers, the responses converter (already gone in Phase C PR-C5), memory subsystem (replaced by live-zone tail injection from Phase A PR-A2 and Phase B PR-B6), semantic cache, batch handler, etc. The Rust proxy becomes the canonical request-path implementation. Operators run only `legroom-proxy` (Rust binary).

### Files

**Delete:**
- `legroom/proxy/server.py` (2864 LOC)
- `legroom/proxy/handlers/anthropic.py` (2423 LOC)
- `legroom/proxy/handlers/openai.py` (2742 LOC)
- `legroom/proxy/handlers/streaming.py` (1131 LOC)
- `legroom/proxy/handlers/gemini.py` (839 LOC)
- `legroom/proxy/handlers/batch.py` (1010 LOC)
- `legroom/proxy/responses_converter.py` — already deleted in PR-C5; verify gone.
- `legroom/proxy/memory_handler.py` (1756 LOC)
- `legroom/proxy/memory_tool_adapter.py` (1273 LOC)
- `legroom/proxy/semantic_cache.py` (142 LOC)
- `legroom/proxy/savings_tracker.py` (934 LOC) — re-implemented in Rust as part of `observability/`.
- `legroom/proxy/loopback_guard.py` (92 LOC) — Rust equivalent in `crates/legroom-proxy/src/handlers/debug.rs`.
- `legroom/proxy/ws_session_registry.py` (226 LOC) — Rust equivalent.
- `legroom/proxy/interceptors/` — all of this directory.
- `legroom/proxy/cost.py`, `helpers.py`, `rate_limiter.py`, `request_logger.py` — re-implemented in Rust as part of compression dispatch / observability.
- `legroom/proxy/prometheus_metrics.py` — re-implemented in Rust as `observability/prometheus.rs`.
- `legroom/proxy/extensions.py`, `models.py`, `modes.py`, `stage_timer.py`, `warmup.py`, `responses_converter.py`, `debug_introspection.py`.
- `legroom/transforms/cache_aligner.py` — already gutted in Phase A PR-A2; delete the remaining stub.

**Move:**
- `legroom/proxy/loopback_guard.py` test logic → `crates/legroom-proxy/tests/integration_loopback_guard.rs`.

**Modify:**
- `legroom/cli/proxy.py` — `legroom proxy start` now spawns the Rust binary (`./target/release/legroom-proxy`) instead of `uvicorn legroom.proxy.server:app`.
- `legroom/cli/wrap/*.py` — same: env-var setup remains, but `proxy_url` points at the Rust binary's listen address.
...
- Manual test: `legroom proxy start` boots the Rust binary; `curl -s http://127.0.0.1:8787/healthz` returns OK.
...
**Worktree:** `~/claude-projects/legroom-worktrees/realign-H2-retire-litellm-backends`
**Risk:** **MEDIUM**
**LOC:** **-3,000 / +100**
...
**Delete:**
- `legroom/backends/litellm.py` (~1500 LOC, the lossy converter)
- `legroom/backends/__init__.py` if the only contents was the LiteLLM backend.

**Modify:**
- `pyproject.toml` — remove `litellm` from dependencies. (Saves ~50 MB of installed-deps size.)
- `legroom/providers/registry.py` — delete `litellm-bedrock`, `litellm-vertex` provider entries.
- `legroom/cli/wrap/*` — verify no wrap CLIs route to LiteLLM (they shouldn't; they go through the proxy).
...
**Worktree:** `~/claude-projects/legroom-worktrees/realign-H3-final-cleanup`
**Risk:** **LOW**
**LOC:** -2,000 / +500
...
- `legroom/__init__.py` — drop unused module imports.
...
- `crates/legroom-parity/` — the parity harness itself becomes irrelevant after Python side is gone. **Decision needed** (see `12-decisions-needed.md` Q3): keep parity-run as a "prior-version-vs-current-version" harness, or delete?

**Add:**
- `CHANGELOG.md` entry: "**Breaking**: Python proxy retired. Operators must use the Rust binary `legroom-proxy`. See migration guide at `docs/operations/python-to-rust-migration.md`."
- `docs/operations/python-to-rust-migration.md` — operator migration guide.

### Acceptance criteria

- `git grep -i "uvicorn\|fastapi" legroom/` returns nothing in non-test code.
...
| Module | Role | Reason |
|---|---|---|
| `legroom/cli/wrap/*.py` | Agent launchers | Off-path; orchestrates filesystem + subprocess. Python is the right tool. |
| `legroom/cli/{evals,init,install,learn,memory,perf,proxy,tools}.py` | CLI admin commands | Click-based; off-path. |
| `legroom/rtk/installer.py` | RTK binary downloader | Off-path; filesystem operations. |
| `legroom/providers/codex/install.py`, `claude/install.py` | Client config installation | Off-path; filesystem. |
| `legroom/evals/`, `learn/`, `memory/` writers | Research / batch tooling | Off-path; long-running batch. |
| `legroom/tokenizers/` | Parity backstop | Used only by parity harness if H3 keeps it. |
| `legroom/telemetry/toin.py` | TOIN learning loop | Off-path; observation-only after Phase B PR-B5. |
| `legroom/subscription/tracker.py`, `client.py` | Subscription usage poller | Off-path. |
| `legroom/copilot_auth.py` | Copilot OAuth refresh | Off-path; specific to Copilot integration. |
...
- ✅ Operators run only the Rust `legroom-proxy` binary
