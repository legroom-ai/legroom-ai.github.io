...
$ImageDefault = 'ghcr.io/legroom-ai/legroom:latest'
$InstallImage = if ($env:LEGROOM_DOCKER_IMAGE) { $env:LEGROOM_DOCKER_IMAGE } else { $ImageDefault }
...
function Ensure-ProfileBlock {
    ...
    $markerStart = '# >>> legroom docker-native >>>'
    $markerEnd = '# <<< legroom docker-native <<<'
...
function Write-Wrapper {
    ...
    $wrapperPath = Join-Path $TargetDir 'legroom.ps1'
    $cmdPath = Join-Path $TargetDir 'legroom.cmd'
    $resolvedInstallImage = $InstallImage.Replace("'", "''")

    $wrapper = @'
$ErrorActionPreference = 'Stop'

$LegroomImage = if ($env:LEGROOM_DOCKER_IMAGE) { $env:LEGROOM_DOCKER_IMAGE } else { '__LEGROOM_INSTALL_IMAGE__' }
$ContainerHome = if ($env:LEGROOM_CONTAINER_HOME) { $env:LEGROOM_CONTAINER_HOME } else { '/tmp/legroom-home' }
$HostHome = $HOME
...
function Get-PassthroughEnvArgs {
    ...
    $prefixes = @(
        'LEGROOM_','ANTHROPIC_','OPENAI_','GEMINI_','AWS_','AZURE_','VERTEX_',
...
function Get-SharedDockerArgs {
    ...
    $args.Add('--env')
    $args.Add("LEGROOM_WORKSPACE_DIR=$ContainerHome/.legroom")
    $args.Add('--env')
    $args.Add("LEGROOM_CONFIG_DIR=$ContainerHome/.legroom/config")
    $args.Add('--volume')
    $args.Add("${PWD}:/workspace")
    $args.Add('--volume')
    $args.Add((Join-Path $HostHome '.legroom') + ":$ContainerHome/.legroom")
    $args.Add('--volume')
...
function Invoke-LegroomDocker {
    ...
    $dockerArgs.Add('legroom')
    $dockerArgs.Add($LegroomImage)
...
function Wait-Proxy {
    ...
    throw "Legroom proxy failed to start on port $Port"
}
...
function Start-ProxyContainer {
    ...
    $containerName = "legroom-proxy-$Port-$PID"
    $dockerArgs = New-Object System.Collections.Generic.List[string]
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to start Legroom proxy container"
    }
...
function Get-PersistentProfileRoot {
    ...
    return Join-Path (Join-Path $HostHome '.legroom\deploy') $Profile
}
...
function Get-ManifestProxyArgs {
    ...
    if ($Memory) {
        $args.AddRange([string[]]@('--memory','--memory-db-path',"$ContainerHome/.legroom/memory.db"))
    }
    if ($AnyllmProvider) {
...
function Write-PersistentManifest {
    ...
    $manifest = [ordered]@{
        ...
        proxy_mode = $Mode
        memory_enabled = $Memory
        memory_db_path = "$ContainerHome/.legroom/memory.db"
        telemetry_enabled = $TelemetryEnabled
        image = $Image
        service_name = "legroom-$Profile"
        container_name = Get-PersistentContainerName -Profile $Profile
        health_url = "http://127.0.0.1:$Port/readyz"
        base_env = $baseEnv
        tool_envs = @{}
...
function Parse-InstallApplyArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'legroom install apply': $arg"
            }
        }
    }
...
function Parse-InstallProfileArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'legroom install': $arg"
            }
        }
    }
...
function Invoke-ClaudeRtkInit {
    $rtkPath = Join-Path $HostHome '.legroom\bin\rtk.exe'
...
function Get-ContextTool {
    $value = $env:LEGROOM_CONTEXT_TOOL
    ...
    if ($value -ne 'rtk' -and $value -ne 'lean-ctx') {
        Fail 'LEGROOM_CONTEXT_TOOL must be one of: lean-ctx, rtk'
    }
    return $value
}
...
function Parse-OpenClawWrapArgs {
    ...
    $pluginPath = $null
    $pluginSpec = 'legroom-ai/openclaw'
    $skipBuild = $false
    $copy = $false
    $proxyPort = 8787
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'legroom wrap openclaw': $arg"
            }
        }
    }
...
function Parse-OpenClawUnwrapArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'legroom unwrap openclaw': $arg"
            }
        }
    }
...
function Get-OpenClawExistingEntryJson {
    $output = (& openclaw config get plugins.entries.legroom 2>$null | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) {
        return $null
    }
    return $output
...
function Copy-OpenClawPluginIntoExtensions {
    ...
    $targetDir = Join-Path $extensionsDir 'legroom'
...
function Invoke-OpenClawWrap {
    ...
    Write-Host ""
    Write-Host "  ╔═══════════════════════════════════════════════╗"
    Write-Host "  ║           LEGROOM WRAP: OPENCLAW             ║"
    Write-Host "  ╚═══════════════════════════════════════════════╝"
    Write-Host ""
    if ($parsed.PluginPath) {
    ...
    [void](Invoke-CapturedCommand -Action 'openclaw config set plugins.entries.legroom' -Command 'openclaw' -Arguments @('config','set','plugins.entries.legroom',$entryJson,'--strict-json'))
    Write-Host '  Installing OpenClaw plugin with required unsafe-install flag...'
    Install-OpenClawPlugin -Parsed $parsed
    [void](Invoke-CapturedCommand -Action 'openclaw config set plugins.slots.contextEngine' -Command 'openclaw' -Arguments @('config','set','plugins.slots.contextEngine','"legroom"','--strict-json'))
    ...
    $inspectOutput = Invoke-CapturedCommand -Action 'openclaw plugins inspect legroom' -Command 'openclaw' -Arguments @('plugins','inspect','legroom')
    if ($parsed.Verbose -and $inspectOutput) {
        Write-Host $inspectOutput
    }

    Write-Host ""
    Write-Host "✓ OpenClaw is configured to use Legroom context compression."
    Write-Host "  Plugin: legroom"
    Write-Host "  Slot:   plugins.slots.contextEngine = legroom"
    Write-Host ""
}
...
function Invoke-OpenClawUnwrap {
    ...
    Write-Host ""
    Write-Host "  ╔═══════════════════════════════════════════════╗"
    Write-Host "  ║          LEGROOM UNWRAP: OPENCLAW            ║"
    Write-Host "  ╚═══════════════════════════════════════════════╝"
    Write-Host ""
    Write-Host '  Disabling Legroom plugin and removing engine mapping...'
    ...
    Write-Host ""
    Write-Host "✓ OpenClaw Legroom wrap removed."
    Write-Host "  Plugin: legroom (installed, disabled)"
...
function Invoke-PrepareOnly {
    ...
    $dockerArgs.AddRange((Get-SharedDockerArgs))
    $dockerArgs.Add('--env')
    $dockerArgs.Add("LEGROOM_RTK_TARGET=$(Get-RtkTarget)")
    $dockerArgs.Add('--entrypoint')
    $dockerArgs.Add('legroom')
    $dockerArgs.Add($LegroomImage)
...
if ($args.Count -eq 0) {
    Invoke-LegroomDocker -Arguments @('--help')
    exit 0
}
...
switch ($args[0]) {
    ...
    'wrap' {
        ...
        if ($args.Count -lt 2) {
            Fail 'Usage: legroom wrap <claude|codex|aider|cursor|openclaw|opencode> [...]'
        }
        ...
        if ($tool -eq 'openclaw') {
            if (Test-HelpFlag -Arguments $wrapArgs) {
                ...
                Invoke-LegroomDocker -Arguments $helpArgs
                exit 0
            }
        ...
        try {
            ...
            switch ($tool) {
                ...
                'cursor' {
                    Write-Host "Legroom proxy is running for Cursor."
                    Write-Host ""
    ...
    'unwrap' {
        if ($args.Count -eq 1 -or $args[1] -eq '--help' -or $args[1] -eq '-?') {
            Invoke-LegroomDocker -Arguments @('unwrap','--help')
            exit 0
        }
        ...
        Invoke-LegroomDocker -Arguments $args
    }
    'proxy' {
        $port = 8787
...
'@
    ...
    $wrapper = $wrapper.Replace('__LEGROOM_INSTALL_IMAGE__', $resolvedInstallImage)

    $cmdWrapper = ([string][char]64) + "echo off`r`npowershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File ""%~dp0legroom.ps1"" %*`r`n"
...
if ($env:LEGROOM_DOCKER_IMAGE) {
    $null = docker image inspect $InstallImage 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Info "Using existing LEGROOM_DOCKER_IMAGE=$InstallImage"
    } else {
...
Write-Host ""
Write-Host "Legroom Docker-native install complete."
Write-Host ""
Write-Host "Installed wrappers:"
Write-Host "  $InstallDir\legroom.ps1"
Write-Host "  $InstallDir\legroom.cmd"
Write-Host ""
Write-Host "Next steps:"
Write-Host "  1. Restart PowerShell"
Write-Host "  2. Try: legroom proxy"
Write-Host "  3. Docs: https://github.com/legroom-ai/legroom-ai.github.io/blob/main/docs/docker-install.md"
