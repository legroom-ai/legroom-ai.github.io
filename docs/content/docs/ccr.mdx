...
Legroom's CCR (Compress-Cache-Retrieve) architecture makes compression **reversible**. When content is compressed, the original data is cached locally. If the LLM needs the full data, it retrieves it instantly.
...
The client never sees CCR tool calls on the Anthropic and OpenAI proxy paths; Legroom resolves them transparently there.
...
<Callout type="warning" title="Current Gemini limitation">
  ...
  `legroom_retrieve` is not resolved transparently on that path today. Google's
  ...
  Anthropic or OpenAI proxy paths. See [issue #2041](https://github.com/legroom-ai/legroom/issues/2041).
</Callout>
...
import type { CCRConfig } from "legroom-ai";
...
const ccrConfig: CCRConfig = {
  enabled: true,
  injectTool: true,             // Inject legroom_retrieve tool
...
# The LLM calls legroom_retrieve when it needs more data.
...
agent runs, set `LEGROOM_CCR_TTL_SECONDS` before starting the proxy:

```bash
LEGROOM_CCR_TTL_SECONDS=7200 legroom proxy
```
