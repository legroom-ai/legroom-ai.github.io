...
description: Solutions for common Legroom issues including proxy startup, connection errors, no token savings, high latency, and installation problems.
---
...
**Fix**: Legroom pins `ORT_DYLIB_PATH` automatically at import time to the
DLL inside the `onnxruntime` pip package (included in `legroom-ai[proxy]`).
...
`LEGROOM_MAGIKA_INIT_TIMEOUT_SECS` (default `5`) bounds the init as a safety
...
**Symptom**: After pointing Claude Code at Legroom (`ANTHROPIC_BASE_URL`), `/context all`
...
```bash
# Easiest: `legroom wrap claude` sets ENABLE_TOOL_SEARCH=true automatically.
legroom wrap claude
...
legroom wrap claude --tool-search auto
...
See [issue #746](https://github.com/legroom-ai/legroom-ai.github.io/issues/746) for the full analysis.
...
<Callout type="warning" title="Claude Code VSCode extension caveat">
  ...
  content blocks that `ENABLE_TOOL_SEARCH=true` enables through Legroom. Tool
  ...
  VSCode, set `ENABLE_TOOL_SEARCH=false` for that target and restart the Legroom
  deployment. See [issue #2028](https://github.com/legroom-ai/legroom/issues/2028).
</Callout>

## Remote Control unavailable through custom ANTHROPIC_BASE_URL

**Symptom**: When Claude Code runs with `ANTHROPIC_BASE_URL` set to a custom host (for example, Legroom), the Remote Control menu is absent.

**Cause**: This is a Claude-side gate. Legroom only receives normal API traffic and can still compress it, but Claude evaluates Remote Control availability before proxy traffic reaches the server.

**Fix**: Use Legroom for normal proxied API sessions, and launch Claude directly (without `ANTHROPIC_BASE_URL`) when you need Claude Remote Control.

`ENABLE_TOOL_SEARCH` is unaffected and can stay enabled for context-window savings while routing through Legroom.
...
### pipx installs an older Legroom version

**Symptom**: PyPI shows a newer `legroom-ai` release, but `pipx install` or
...
environment uses a Python version that Legroom does not publish wheels for yet,
...
If you already have Legroom installed under `pipx`, uninstall it first or
...
**Symptom**: On Windows, `uv tool install "legroom-ai[all]"` (or `pip install`) fails while installing the `ast-grep-cli` wheel, and Windows Defender flags `sg.exe`:
...
**Cause**: A known **false positive** in the upstream `ast-grep-cli` wheel's bundled `sg.exe` ([ast-grep/ast-grep#2799](https://github.com/ast-grep/ast-grep/issues/2799)), not a Legroom issue. `ast-grep` is a base dependency, so the block also affects the `[proxy]` extra. Legroom uses `ast-grep` only for optional AST-based Read-output outlining and **runs normally without it** — the only impact is the install-time quarantine.

**Workarounds** (safest first):

1. **Run the proxy in Docker** — no local wheel is installed, so Defender is never triggered. See [Docker install](/docs/docker-install); the image is `ghcr.io/legroom-ai/legroom`.
...
4. File an issue at [github.com/legroom-ai/legroom-ai.github.io](https://github.com/legroom-ai/legroom-ai.github.io/issues) with your Legroom version, Python version, provider, debug log output, and minimal reproduction code
