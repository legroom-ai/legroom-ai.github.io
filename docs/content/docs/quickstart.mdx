---
title: Quickstart
description: Get Legroom running in 5 minutes. Install, compress, and send to your LLM with fewer tokens.
---
...
The TypeScript SDK sends messages to a local Legroom proxy for compression. Start the proxy before using the TS SDK:

```bash
uv tool install --python 3.13 "legroom-ai[proxy]"
# or, inside a Python project: pip install "legroom-ai[proxy]"
...
If you do not want to change any code, run Legroom as a proxy and point your existing client at it:
...
All requests flow through Legroom automatically. Check savings at any time:
...
The biggest savings come from tool outputs -- search results, database rows, log files, API responses. Legroom auto-detects the content type and routes it to the best compressor. No configuration needed.
