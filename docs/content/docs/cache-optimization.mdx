...
LLM providers cache prompt prefixes to avoid reprocessing identical input on repeated calls. Legroom's **CacheAligner** is detector-only, so it surfaces prefix drift, reports observability data, and leaves message assembly to the caller.
...
Each LLM provider implements caching differently. Legroom applies the optimal strategy for each.
...
Keep the stable prefix byte-identical, then place provider cache markers where your client or orchestrator already assembles the request. Legroom's job is to surface prefix instability, not repair it.
...
Keep the prefix stable in your integration layer; Legroom reports when the cacheable zone drifts. The CachedContent lifecycle itself also stays in your integration layer.
