...
- `legroom_retrieve` for on-demand recovery of stored originals
- `LEGROOM_CCR_TTL_SECONDS` for sizing the local store lifetime
- `compression_strategy` as the authoritative discriminator on stored CCR entries

For routing decisions, the same rule in plain terms is: legroom_retrieve recovers originals, LEGROOM_CCR_TTL_SECONDS sizes the local lifetime, compression_strategy identifies the producing path, and shape inference is not the routing authority.
...
| Mode | Use when | Notes |
|---|---|---|
| Proxy | You want spawned agents and normal client traffic to pass through a local provider endpoint | Good for `legroom proxy --mode cache` and provider base URL routing |
...
| MCP | Agents need on-demand compression and retrieval tools | Best when `legroom_retrieve` should be available as a tool |
