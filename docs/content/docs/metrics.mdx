...
description: Monitor compression performance, cost savings, and system health with Legroom's built-in metrics, Prometheus endpoint, and SDK APIs.
---

Legroom provides comprehensive metrics for monitoring compression performance, cost savings, and system health through both the proxy server and the SDK.
...
Persistent savings are stored at `~/.legroom/proxy_savings.json` and survive proxy restarts. Override the path with `LEGROOM_SAVINGS_PATH`.
...
```
# HELP legroom_requests_total Total requests processed
legroom_requests_total{mode="optimize"} 1234

# HELP legroom_tokens_saved_total Total tokens saved
legroom_tokens_saved_total 5678900

# HELP legroom_persistent_savings_tokens_saved_total Durable lifetime input tokens saved by proxy compression
legroom_persistent_savings_tokens_saved_total 5678900

# HELP legroom_compression_ratio Compression ratio histogram
legroom_compression_ratio_bucket{le="0.5"} 890
legroom_compression_ratio_bucket{le="0.7"} 1100
legroom_compression_ratio_bucket{le="0.9"} 1200

# HELP legroom_latency_seconds Request latency histogram
legroom_latency_seconds_bucket{le="0.01"} 800
legroom_latency_seconds_bucket{le="0.1"} 1150

# HELP legroom_cache_hits_total Cache hit counter
...
|--------|-------------------|--------|
| `legroom_tokens_saved_total` | Runtime tokens saved since this proxy process started | Higher is better |
| `legroom_persistent_savings_tokens_saved_total` | Durable lifetime tokens saved from `/stats.persistent_savings` | Higher is better |
...
[`examples/grafana/legroom-dashboard.json`](https://github.com/legroom-ai/legroom/blob/main/examples/grafana/legroom-dashboard.json).
...
(`legroom_overhead_ms_sum` / `legroom_overhead_ms_count`) straight from the metrics
...
| Panel | PromQL |
|-------|--------|
| Runtime Tokens Saved | `legroom_tokens_saved_total` |
| Lifetime Tokens Saved | `legroom_persistent_savings_tokens_saved_total` |
| Compression Ratio (median) | `histogram_quantile(0.5, legroom_compression_ratio_bucket)` |
| Request Latency (p99) | `histogram_quantile(0.99, legroom_latency_seconds_bucket)` |
| Cache Hit Rate | `legroom_cache_hits_total / (legroom_cache_hits_total + legroom_cache_misses_total)` |
