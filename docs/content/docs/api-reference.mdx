---
title: API Reference
description: Complete API reference for the Legroom Python and TypeScript SDKs. Core client, configuration types, result types, errors, and utilities.
---

Complete API reference for the Legroom Python and TypeScript SDKs.

## Core

### LegroomClient

The main entry point for the Legroom SDK.

<Tabs groupId="lang" items={['TypeScript', 'Python']}>
<Tab value="TypeScript">

<TypeTable type={{
  baseUrl: { type: 'string', description: 'Base URL for the Legroom proxy' },
...
Accepts all standard OpenAI/Anthropic parameters plus Legroom-specific overrides:

<TypeTable type={{
  legroom_mode: { type: '"audit" | "optimize" | "simulate"', description: 'Override mode for this request', default: 'Client default' },
  legroom_query: { type: 'str', description: 'Query for relevance scoring', default: 'None' },
  legroom_output_buffer_tokens: { type: 'int', description: 'Reserve tokens for output', default: '4000' },
  legroom_keep_turns: { type: 'int', description: 'Keep last N turns uncompressed', default: '2' },
  legroom_tool_profiles: { type: 'dict', description: 'Per-tool compression overrides', default: '{}' },
}} />
...
Top-level function to compress messages via the Legroom proxy.
...
<TypeTable type={{
  ...
  fallback: { type: 'boolean', description: 'Return original messages on failure instead of throwing' },
  retries: { type: 'number', description: 'Number of retry attempts on failure' },
  client: { type: 'LegroomClientInterface', description: 'Pre-configured client instance to use' },
...
Context management is now handled automatically inside the pipeline (live-zone-only compression). Legroom never drops messages from the conversation history; it compresses only the newest content blocks (latest user message, latest tool result) and keeps the cache hot zone — system prompt, tools, and older turns — untouched. Use the `legroom_keep_turns` / `legroom_output_buffer_tokens` per-request overrides to tune behavior. The `RollingWindowConfig`, `IntelligentContextConfig`, and `ScoringWeights` classes are no longer part of Legroom.
...
<TypeTable type={{
  ...
  defaultMode: { type: 'LegroomMode', description: 'Default compression mode' },
...
# Note: rolling_window has been removed — use legroom_keep_turns per-request instead
```
...
Semantic similarity scoring (requires `legroom-ai[relevance]`):
...
| Exception | Meaning |
|-----------|---------|
| `LegroomError` | Base class for all errors |
| `LegroomConnectionError` | Cannot reach proxy |
| `LegroomAuthError` | 401 from proxy |
| `LegroomCompressError` | Compression failed (includes `statusCode`, `errorType`) |
...
| Exception | Meaning |
|-----------|---------|
| `LegroomError` | Base class for all Legroom errors |
