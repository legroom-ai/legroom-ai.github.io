---
title: Architecture
description: How Legroom's three-stage compression pipeline works, from message parsing through transform execution to provider cache optimization.
---

Legroom sits between your application and the LLM provider. It intercepts messages, compresses them intelligently, and forwards the optimized request. The response comes back unchanged.
...
Legroom can be used in three ways, all feeding into the same pipeline:

| Entry Point | How It Works | Code Changes |
|-------------|-------------|--------------|
| **SDK Mode** | Wrap your LLM client with `LegroomClient` | Minimal -- swap client constructor |
| **Proxy Mode** | Run `legroom proxy` and point your client at it | Zero -- just change the base URL |
...
After the pipeline, Legroom applies provider-specific cache hints:
...
When SmartCrusher compresses a tool output or Intelligent Context drops messages, the original content is stored in a local compression cache. If the LLM needs the full data, it can request retrieval via a `legroom_retrieve` tool call. This makes compression reversible.

```
Compress:  1000 items  ->  15 items  (stored original in CCR)
Cache:     Hash-indexed local store (SQLite)
Retrieve:  LLM calls legroom_retrieve("abc123")  ->  original 1000 items
```
