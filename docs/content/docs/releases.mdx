...
Legroom uses `release-please` to maintain a release PR from conventional commits on `main`. Merging that release PR creates the release tag and GitHub Release, which triggers `.github/workflows/release.yml` to publish all packages, build version-matched Docker images, and attach release assets.
...
|---------|------|----------|----------------------|
| `legroom-ai` | Python | PyPI | `PYPI_PACKAGE` |
| `legroom-ai` | TypeScript SDK | npmjs.org | `NPM_SDK_PACKAGE` |
| `legroom-openclaw` | TypeScript plugin | npmjs.org | `NPM_OPENCLAW_PACKAGE` |
| `@{owner}/legroom-ai` | TypeScript SDK | GitHub Package Registry | — |
| `@{owner}/legroom-openclaw` | TypeScript plugin | GitHub Package Registry | — |
| `legroom-ai-{version}.tar.gz` / `legroom_ai-{version}-py3-none-any.whl` | Python package distributions | GitHub Release (`{owner}/legroom`) | — |
| `legroom-ai-{version}.tgz` / `legroom-openclaw-{version}.tgz` | Node release assets | GitHub Release (`{owner}/legroom`) | — |
| `ghcr.io/{owner}/legroom` | Docker image | GitHub Container Registry | — |
...
- `legroom/_version.py` - `__version__`, synced at build time
...
### smoke-import-wheels
Installs the built wheels into representative customer environments and imports `legroom._core`. This blocks publishing if a wheel builds successfully but cannot import on its promised platform floor.
...
- `sdk/typescript/` as `legroom-ai`
- `plugins/openclaw/` as `legroom-openclaw`
...
- `sdk/typescript/` as `@{owner}/legroom-ai`
- `plugins/openclaw/` as `@{owner}/legroom-openclaw`
...
```yaml
env:
  PYPI_PACKAGE: legroom-ai
  PYPI_ENVIRONMENT: pypi
  NPM_REGISTRY_URL: https://registry.npmjs.org
  NPM_SDK_PACKAGE: legroom-ai
  NPM_OPENCLAW_PACKAGE: legroom-openclaw
