...
Legroom integrates with [CrewAI](https://github.com/crewAIInc/crewAI) to compress tool outputs before they enter the agent's LLM context. Tool-heavy agents that return large JSON arrays, database results, or verbose logs see 60-90% token reduction.

## Installation

```bash
pip install legroom-ai crewai
```
...
wrapped = wrap_tools_with_legroom([search_database])
...
```python
from legroom.integrations.crewai import get_tool_metrics
...
```python
from legroom.integrations.crewai import reset_tool_metrics
...
```python
from legroom.integrations.crewai import ToolMetricsCollector, wrap_tools_with_legroom
...
`LegroomToolWrapper` subclasses `BaseTool` and overrides `_run()` to:

1. Call the original tool's `run()` method
2. Check if the output exceeds `min_chars_to_compress`
3. If so, compress via Legroom's `compress_tool_result()`
