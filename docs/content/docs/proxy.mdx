---
title: Proxy Server
description: Run the Legroom proxy to compress LLM traffic for any client — Claude Code, Cursor, OpenAI SDK, or custom apps.
---

The Legroom proxy is a standalone HTTP server that compresses all LLM traffic passing through it. Point any client at the proxy and get automatic context optimization.
...
legroom proxy --host 0.0.0.0 --port 8080
...
Telemetry is **off by default** (opt-in). Opt in with `LEGROOM_TELEMETRY=on` or `--telemetry`.
...
Use `--http-proxy` or `LEGROOM_HTTP_PROXY` when only provider API traffic should go through a proxy:

```bash
legroom proxy --http-proxy http://proxy.internal:8080
```

Avoid setting process-wide variables such as `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, or `NO_PROXY` for this use case. HTTPX reads those variables too, but Legroom also inherits them into tool executions, so they can proxy unrelated tool traffic.
...
| `--code-aware` / `--no-code-aware` | disabled | Enable or disable AST-based code compression. Requires `legroom-ai[code]`. |
| `--code-graph` | `false` | Force a tokensave code-graph index of the current project (tokensave is the default coding-task compressor registered by `legroom wrap`). |

#### tokensave binary trust model

`legroom wrap` registers **tokensave** (a local code-graph MCP server) as the default coding-task compressor. tokensave ships as a single prebuilt Rust binary, so `wrap` downloads the release asset for your platform from GitHub and runs it locally. Because the binary is executed, every supported asset is **pinned to a SHA-256 digest in Legroom** (`legroom/graph/tokensave_installer.py`); the downloaded bytes are verified against that digest before extraction, and a mismatch aborts the install (Legroom falls back to the Serena backup) rather than running unverified code.

- Set `LEGROOM_BINARIES_OFFLINE=1` to never reach the network — `wrap` then uses an already-installed tokensave or falls back to Serena.
- `LEGROOM_TOKENSAVE_VERSION` overrides the pinned release tag. Since an overridden version has no pinned digest, the download is **refused** unless you also set `LEGROOM_TOKENSAVE_ALLOW_UNVERIFIED=1`.
...
`LEGROOM_SAVINGS_PROFILE` selects a named profile that seeds Legroom's whole compression posture — proxy mode, keep-ratio, which messages are compressed, and `force_kompress` — at proxy startup. It is read by `legroom proxy` and by the `legroom wrap` subprocesses. When unset, the default profile is `coding`.
...
An unrecognized `LEGROOM_SAVINGS_PROFILE` value logs a warning and falls back to `balanced` — the proxy never fails to start over a bad profile name. See `legroom/agent_savings.py` for each profile's full set of knobs.

Because the default `coding` profile uses **cache** mode (and the proxy's own default mode is also `cache`), Legroom runs in cache mode out of the box. Mode precedence: an explicit `--mode` wins, otherwise `LEGROOM_MODE` (which a profile seeds), otherwise the `cache` default. To run token mode, pass `--mode token` or choose a token-mode profile:

```bash
# Aggressive ~90% token-savings profile
LEGROOM_SAVINGS_PROFILE=agent-90 legroom proxy --port 8787
```
...
| `--memory-db-path` | `{cwd}/.legroom/memory.db` | Override the memory SQLite path |
...
| `--compress-passthrough` | `false` | Also compress custom proxy paths that fall through to the catch-all handler (OpenAI Responses-shaped bodies, path ends in `/responses`). Also `LEGROOM_COMPRESS_PASSTHROUGH=1` |

```bash
legroom proxy --memory
legroom proxy --learn --min-evidence 3
legroom proxy --codex-wire-debug
legroom proxy --compress-passthrough
```

<Callout type="info" title="LLMLingua removed from the proxy CLI">
The old LLMLingua proxy toggles are no longer part of the CLI. Legroom's proxy compression path uses ContentRouter plus the current built-in compressors, including Kompress where applicable.
</Callout>

## Savings profiles

The proxy uses a **savings profile** to control compression behavior — which messages get compressed, how aggressively, and whether to prioritize provider prefix-cache stability or raw savings. Only the env var survives across related tools (`legroom wrap` passes it to the proxy it launches).

```bash
# Switch to a different profile
LEGROOM_SAVINGS_PROFILE=agent-90 legroom proxy
```
...
legroom proxy
legroom proxy --mode token          # --mode token is silently overridden
```

To run in token mode, switch to a profile that uses it:

```bash
LEGROOM_SAVINGS_PROFILE=agent-90 legroom proxy --mode token
```
...
LEGROOM_SAVINGS_PROFILE=coding LEGROOM_FORCE_KOMPRESS=1 legroom proxy

# Start from balanced but lower the keep-ratio
LEGROOM_SAVINGS_PROFILE=balanced LEGROOM_TARGET_RATIO=0.15 legroom proxy
```

### Custom profiles

For permanent custom profiles, see the profile definitions in `legroom/agent_savings.py`. Each profile is an `AgentSavingsProfile` dataclass with fields for compression mode, target ratio, turn protection, and pipeline toggles.
...
Live session statistics plus durable `persistent_savings` totals. Stored at `~/.legroom/proxy_savings.json` (override with `LEGROOM_SAVINGS_PATH`).
...
```
legroom_requests_total{mode="optimize"} 1234
legroom_tokens_saved_total 5678900
legroom_persistent_savings_tokens_saved_total 5678900
legroom_compression_ratio_bucket{le="0.5"} 890
legroom_latency_seconds_bucket{le="0.01"} 800
legroom_cache_hits_total 456
```

`legroom_tokens_saved_total` is the runtime counter for the current proxy process. Use `legroom_persistent_savings_tokens_saved_total` for durable lifetime savings that match `/stats.persistent_savings`.
...
Set `x-legroom-bypass: true` to skip compression.

## Agent wrapping

Use `legroom wrap` to launch supported CLI agents through the local proxy:
...
Cursor reads model endpoints from its settings UI, so `legroom wrap cursor`
...
Grok Build reads model endpoints from `~/.grok/config.toml`. `legroom wrap grok-build`
...
`--bedrock-api-url` lets Legroom sit in that chain. It registers passthrough routes for `/model/{id}/invoke` and `/model/{id}/invoke-with-response-stream`, compresses the request body with the same pipeline as `/v1/messages`, and forwards to the gateway:

```bash
legroom proxy --bedrock-api-url http://127.0.0.1:4000
# then point the client's Bedrock endpoint at Legroom:
...
`build-essential` is required at install time because `legroom-ai` includes `hnswlib`, a C++ extension compiled from source. It is removed after installation to keep the image slim.
</Callout>
