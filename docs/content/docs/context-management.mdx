...
Context management is now handled automatically inside the pipeline (live-zone-only compression). Legroom **never drops messages** from the conversation history and does not do position-based or score-based context management.

## How It Works

Legroom compresses only the **newest content blocks** — the latest user message and the latest tool result / tool output. Compression is type-aware and reversible via [CCR](/docs/ccr), so the LLM can retrieve the original content on demand.
...
Legroom enforces several protections to ensure model output quality:
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=messages,
    legroom_output_buffer_tokens=8000,  # More room for long responses
    legroom_keep_turns=5,               # Protect last 5 turns
)
```
</Tab>
</Tabs>

> **Note:** The `IntelligentContextConfig`, `ScoringWeights`, and `RollingWindowConfig` classes are no longer part of Legroom. Context management is now handled automatically inside the pipeline (live-zone-only compression).
