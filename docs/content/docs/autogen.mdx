...
Legroom integrates with [AutoGen](https://github.com/microsoft/autogen) (`autogen-agentchat` >=0.7) to compress tool outputs before they enter the agent's model context. Tool-heavy agents that return large JSON arrays, database results, or verbose logs see 60-90% token reduction.

## Installation

```bash
pip install legroom-ai autogen-agentchat
```
...
```python
from legroom.integrations.autogen import get_tool_metrics
...
```python
from legroom.integrations.autogen import reset_tool_metrics
...
```python
from legroom.integrations.autogen import ToolMetricsCollector, wrap_tools_with_legroom
...
`LegroomToolWrapper` creates a new `FunctionTool` with a wrapper function that:

1. Calls the original function
2. Checks if the stringified output exceeds `min_chars_to_compress`
3. If so, compresses via Legroom's `compress_tool_result()`
