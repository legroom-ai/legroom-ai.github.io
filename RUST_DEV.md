# Legroom Rust Rewrite — Developer Guide

This document covers the Rust port of Legroom. It is the only new top-level
...
crates/
  legroom-core/                 # library: shared types + transform trait surface
  legroom-proxy/                # binary: axum /healthz (Phase 2 grows this)
  legroom-py/                   # PyO3 cdylib exposing `legroom._core`
  legroom-parity/               # lib + `parity-run` CLI for Python parity tests
tests/parity/
...
`legroom-py` from `cargo run`/bare-`cargo test` flows so that `cargo test
...
| `make test-parity` | Builds `legroom-py` via maturin, runs `parity-run run` |
| `make bench` | `cargo bench --workspace` |
| `make build-proxy` | Release-builds `legroom-proxy`, strips, prints size |
| `make build-wheel` | `maturin build --release -m crates/legroom-py/pyproject.toml` |
| `make fmt` | `cargo fmt --all` |
| `make lint` | `cargo fmt --check` + `cargo clippy --workspace -- -D warnings` |

## Running the proxy

`legroom-proxy` is a transparent reverse proxy. Phase 1 forwards HTTP/1.1,
...
proxy on a private port and put `legroom-proxy` on the public port pointed
at it; end users notice nothing.

```bash
# Build
make build-proxy
./target/release/legroom-proxy --help

# Run against a local upstream
./target/release/legroom-proxy \
    --listen 0.0.0.0:8787 \
...
LEGROOM_HOST=127.0.0.1 LEGROOM_PORT=8788 python -m legroom.proxy &  # or your existing launcher

# 2. Run the Rust proxy on the previously-public port (8787) pointing at it
./target/release/legroom-proxy --listen 0.0.0.0:8787 --upstream http://127.0.0.1:8788 &
...
| --- | --- | --- | --- |
| `--listen` | `LEGROOM_PROXY_LISTEN` | `0.0.0.0:8787` | bind address |
| `--upstream` | `LEGROOM_PROXY_UPSTREAM` | (required) | base URL the proxy forwards to |
...
`/stats` (`legroom.proxy.prometheus_metrics`):
...
`legroom-py` is a PyO3 cdylib that exposes `legroom._core` in Python. The
...
pip install maturin
cd crates/legroom-py
maturin develop           # editable dev build, installs legroom._core
cd /tmp                   # IMPORTANT: step out of the repo root first
python -c "from legroom._core import hello; print(hello())"
# => legroom-core
```

> Why `cd /tmp`? The repo root also contains the Python `legroom/` package.
> Running the smoke import from the repo root makes Python resolve `legroom`
> to `./legroom/__init__.py` (the full SDK, which pulls in heavy deps) instead
> of the lightweight namespace package installed by maturin. Tests should
> either run outside the repo root, or ensure `legroom` is installed into
...
`crates/legroom-parity` owns the Rust-vs-Python oracle:
...
- `parity-run` CLI: `cargo run -p legroom-parity -- run [--only TRANSFORM]`.
- Unit tests in `crates/legroom-parity/src/lib.rs` include a **negative
...
trait + tier system in `crates/legroom-core/src/signals/`. See
...
`crates/legroom-core/src/pipeline/`. Existing compressors get
...
- **`cache_aligner` fixtures**: `CacheAligner.apply()` takes
  ...
  requires pulling `legroom.providers.*` which imports the full observability
...
- **Pre-commit hook noise**: `scripts/sync-plugin-versions.py` mutates
  ...
  `plugins/legroom-agent-hooks/**/plugin.json` on every commit. Those
...
`crates/legroom-core/src/ccr/backends/` ships three implementations of
the `CcrStore` trait:
...
1. **Python `CompressionStore`** — defaults to `InMemoryBackend` (per-process)
   when `LEGROOM_CCR_BACKEND` is unset. Each worker has its own singleton; CCR
   markers written on worker A are invisible to worker B. Set
   `LEGROOM_CCR_BACKEND=sqlite` to use a shared cross-worker store.
2. **`LegroomProxy._compression_caches`** (`legroom/proxy/server.py`)
   — per-session `CompressionCache` dict (instance var, always per-worker).
3. **`LegroomProxy.session_tracker_store`** — per-session prefix-tracker
   state derived from Anthropic's `cache_read_input_tokens` responses
   (instance var, always per-worker).
4. **TOIN learner state** — writes snapshots to `~/.legroom/toin.json` but
...
When `LEGROOM_CCR_BACKEND` is unset (default InMemoryBackend), the warning
includes CCR retrieval failures and suggests setting `LEGROOM_CCR_BACKEND=sqlite`.
