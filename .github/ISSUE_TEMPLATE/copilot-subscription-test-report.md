...
about: Report results of testing `legroom wrap copilot --subscription` on Linux/Windows/macOS
...
- **How you installed legroom**: (pipx/pip `--pre` wheel · Docker install.sh/ps1 · built from source)
...
- **Command run**:
  ...
  ```
- **Did it print `LEGROOM_OK`?**: yes / no
...
(logs from `~/.legroom/logs/proxy.log`, surprises, etc.)
