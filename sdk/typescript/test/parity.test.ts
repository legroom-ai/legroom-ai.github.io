 ...
 * to the Python SDK when both talk to the same Legroom proxy.
 *
 * Requires:
 *   - LEGROOM_INTEGRATION=1 (env flag)
 *   - legroom proxy running on localhost:8787
 *   - OPENAI_API_KEY set in environment
 *
 * Run: LEGROOM_INTEGRATION=1 OPENAI_API_KEY=sk-... npx vitest run test/parity.test.ts
 */
import { describe, it, expect, beforeAll } from "vitest";
import { compress, LegroomClient, SharedContext, CompressionHooks, simulate } from "../src/index.js";
import type { CompressResult, CompressEvent, CompressContext } from "../src/index.js";
import { execSync } from "child_process";

const INTEGRATION = process.env.LEGROOM_INTEGRATION === "1";
...
`;
  ...
  describe("LegroomClient proxy API parity", () => {
