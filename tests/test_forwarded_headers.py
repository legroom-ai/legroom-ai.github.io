"""Tests for ``legroom.proxy.forwarded_headers`` — Phase F PR-F4.
...
(``LEGROOM_PROXY_TRUSTED_GATEWAY_CIDRS`` unset / empty) is
...
def test_non_allowlisted_peer_ignores_forwarded_and_logs(
    ...
    with caplog.at_level(logging.WARNING, logger="legroom.proxy.forwarded_headers"):
