"""Tests for `legroom wrap openclaude` command (issue #1411)."""
...
def test_wrap_openclaude_routes_proxy_envs(
    ...
    with patch("legroom.cli.wrap.shutil.which", return_value=OPENCLAUDE_BINARY):
...
def test_wrap_openclaude_default_rtk_injects_instructions(
    ...
    with patch("legroom.cli.wrap.shutil.which", return_value=OPENCLAUDE_BINARY):
        with patch("legroom.cli.wrap._ensure_rtk_binary", return_value=tmp_path / RTK_BINARY):
            with patch.object(
                Path,
                "write_text",
                autospec=True,
