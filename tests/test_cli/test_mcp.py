...
def mock_claude_config_path(temp_claude_dir):
    ...
    with patch("legroom.cli.mcp.MCP_CONFIG_PATH", config_path):
        with patch("legroom.cli.mcp.CLAUDE_CONFIG_DIR", temp_claude_dir):
            yield config_path
...
class FakeRegistrar:
    ...
    def get_server(self, server_name: str) -> ServerSpec | None:
        if self.configured and server_name == "legroom":
            return ServerSpec(name="legroom", command="legroom", args=("mcp", "serve"))
        return None
...
class TestMCPConfigFunctions:
    ...
    def test_get_legroom_command_returns_list(self):
        """Command should be a list suitable for subprocess."""
        cmd = get_legroom_command()
    ...
    def test_save_config_creates_directory(self, tmp_path):
        ...
        with patch("legroom.cli.mcp.MCP_CONFIG_PATH", config_path):
            with patch("legroom.cli.mcp.CLAUDE_CONFIG_DIR", claude_dir):
...
class TestMCPUninstallCommand:
    """Test 'legroom mcp uninstall' command."""

    def test_uninstall_removes_legroom(self, mock_mcp_available):
        """Uninstall removes legroom through detected registrars."""
        registrar = FakeRegistrar()

        runner = CliRunner()
        with patch("legroom.mcp_registry.get_all_registrars", return_value=[registrar]):
            result = runner.invoke(main, ["mcp", "uninstall"])

        assert result.exit_code == 0
        assert "removed" in result.output.lower()
        assert registrar.removed == ["legroom"]

    def test_uninstall_checks_only_legroom_servers(self):
        """Uninstall only asks registrars to remove Legroom-owned servers."""
        registrar = FakeRegistrar()
    ...
    def test_uninstall_no_config_file(self, mock_claude_config_path):
        ...
        runner = CliRunner()
        with patch("legroom.mcp_registry.get_all_registrars", return_value=[]):
            result = runner.invoke(main, ["mcp", "uninstall"])
    ...
    def test_uninstall_not_configured(self, mock_claude_config_path):
        """Uninstall when legroom not in config exits cleanly."""
...
class TestMCPStatusCommand:
    """Test 'legroom mcp status' command."""
    ...
    def test_status_configured(self, mock_claude_config_path, mock_mcp_available):
        """Status reports configured when a registrar has legroom."""
...
class TestMCPServeCommand:
    """Test 'legroom mcp serve' command."""
    ...
    def test_serve_defaults_to_stdio(self):
        ...
        runner = CliRunner()
        with patch("legroom.ccr.mcp_server.create_ccr_mcp_server", return_value=fake_server):
...
class TestMCPServerInitialization:
    ...
    def test_mcp_server_has_correct_tool_name(self):
        """MCP server is configured for legroom_retrieve tool."""
        from legroom.ccr.mcp_server import create_ccr_mcp_server
        ...
        assert server.server.name == "legroom"
        # The tool name should be legroom_retrieve
        assert CCR_TOOL_NAME == "legroom_retrieve"
