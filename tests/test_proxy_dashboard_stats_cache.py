...
from legroom.proxy import helpers as proxy_helpers
...
def _reset_rtk_stats_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("LEGROOM_RTK_GAIN_SCOPE", raising=False)
...
def test_get_rtk_stats_can_read_project_scoped_gain(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("LEGROOM_RTK_GAIN_SCOPE", "project")
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/bin/rtk")
    monkeypatch.setattr(subprocess, "run", _fake_run)
...
def test_get_rtk_stats_invalid_scope_defaults_to_global(
    ...
    monkeypatch.setenv("LEGROOM_RTK_GAIN_SCOPE", "workspace")
...
def test_get_context_tool_stats_reads_lean_ctx_gain(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(proxy_helpers.time, "monotonic", lambda: now["value"])
    monkeypatch.setattr(
        "legroom.lean_ctx.get_lean_ctx_path",
...
def test_cost_merge_uses_generic_cli_filtering_name() -> None:
    from legroom.proxy.cost import merge_cost_stats
...
def test_session_summary_uses_generic_cli_filtering_keys() -> None:
    from legroom.proxy.cost import build_session_summary
...
def test_dashboard_session_metrics_do_not_repeat_proxy_tokens_without_new_context() -> None:
    ...
    assert '<span class="text-sm text-gray-400">Legroom Overhead</span>' not in html
...
def test_proxy_throughput_in_stats_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    `from legroom.perf.analyzer import ...` on every call, so we patch the
    names directly on the `legroom.perf.analyzer` module so the local import
    inside the closure picks up our fakes.

    Skipped locally when legroom._core (Rust extension) is not compiled.
    """
    pytest.importorskip("fastapi")
    from fastapi.testclient import TestClient

    import legroom.perf.analyzer as _analyzer_mod
    ...
    try:
        ...
        pytest.skip(f"legroom._core not available (Rust extension not compiled): {exc}")

    from legroom.config import ProxyConfig
