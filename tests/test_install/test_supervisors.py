...
from legroom.install.models import DeploymentManifest, SupervisorKind
...
def _manifest(
    *,
    ...
    return DeploymentManifest(
        ...
        host="127.0.0.1",
        backend="anthropic",
        service_name=f"legroom-{profile}",
...
def test_linux_service_unit_uses_user_systemd_path(monkeypatch, tmp_path: Path) -> None:
    ...
    unit_path, content = _linux_service_unit(manifest, tmp_path / "run-legroom.sh")

    assert unit_path == tmp_path / ".config" / "systemd" / "user" / "legroom-default.service"
    assert "ExecStart=" + str(tmp_path / "run-legroom.sh") in content
    assert "Restart=on-failure" in content


def test_command_for_script_and_unix_runner(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "legroom.install.supervisors.resolve_legroom_command",
        lambda: ["python", "-m", "legroom"],
    )
    ...
    record = _render_unix_runner(
        tmp_path / "scripts" / "run-legroom.sh", ["legroom", "run", "--flag"]
    )
    assert record.kind == "script"
    content = Path(record.path).read_text(encoding="utf-8")
    assert content.startswith("#!/usr/bin/env bash")
    assert "exec legroom run --flag" in content


def test_render_unix_runner_exports_env_before_exec(tmp_path: Path) -> None:
    record = _render_unix_runner(
        tmp_path / "run-legroom.sh",
        ["legroom", "run"],
        {"LEGROOM_WORKSPACE_DIR": "/Users/x/.legroom-workspace", "AWS_PROFILE": "sso-bedrock"},
    )

    content = Path(record.path).read_text(encoding="utf-8")
    export_index = content.index("export LEGROOM_WORKSPACE_DIR=")
    exec_index = content.index("exec legroom run")

    assert "export LEGROOM_WORKSPACE_DIR=/Users/x/.legroom-workspace" in content
...
def test_render_unix_runner_rejects_invalid_env_name(tmp_path: Path) -> None:
    with pytest.raises(click.ClickException, match="Invalid environment variable name"):
        _render_unix_runner(tmp_path / "run-legroom.sh", ["legroom", "run"], {"BAD-NAME": "x"})


def test_render_unix_runner_omits_export_block_without_env(tmp_path: Path) -> None:
    record = _render_unix_runner(tmp_path / "run-legroom.sh", ["legroom", "run"])
...
def test_linux_task_spec_for_user_scope_includes_crontab_markers(tmp_path: Path) -> None:
    ...
    cron_path, content = _linux_task_spec(manifest, tmp_path / "ensure-legroom.sh")

    assert cron_path is None
    assert "# >>> legroom smoke >>>" in content
    assert "# <<< legroom smoke <<<" in content
...
def test_macos_launchd_plist_switches_between_keepalive_and_interval(
    ...
    service_path, service_content = _macos_launchd_plist(
        service_manifest, tmp_path / "run-legroom.sh"
    )
    assert service_path == tmp_path / "Library" / "LaunchAgents" / "com.legroom.default.plist"
    ...
    task_path, task_content = _macos_launchd_plist(
        task_manifest, tmp_path / "ensure-legroom.sh", interval=300
    )
    assert task_path == tmp_path / "Library" / "LaunchAgents" / "com.legroom.tasky.plist"
...
def test_render_windows_runner_writes_ps1_and_cmd_wrappers(tmp_path: Path) -> None:
    ps1_path = tmp_path / "run-legroom.ps1"
    cmd_path = tmp_path / "run-legroom.cmd"

    records = _render_windows_runner(
        ps1_path,
        cmd_path,
        ["C:\\Program Files\\Python\\python.exe", "legroom", "install", "agent", "run"],
    )
    ...
    assert '& "C:\\Program Files\\Python\\python.exe" legroom install agent run' in ps1_content
    assert (
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-legroom.ps1" %*'
        in cmd_content
    )
...
def test_render_windows_runner_rejects_invalid_env_name(tmp_path: Path) -> None:
    with pytest.raises(click.ClickException, match="Invalid environment variable name"):
        _render_windows_runner(
            tmp_path / "run-legroom.ps1",
            tmp_path / "run-legroom.cmd",
            ["legroom", "run"],
            {"BAD-NAME": "x"},
        )
...
def test_render_runner_scripts_writes_unix_scripts(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr(
        "legroom.install.supervisors.resolve_legroom_command", lambda: ["legroom"]
    )
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    manifest = _manifest()
...
def test_render_runner_scripts_threads_base_env_into_both_scripts(
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    manifest = _manifest(base_env={"LEGROOM_WORKSPACE_DIR": "/custom/workspace"})

    records = render_runner_scripts(manifest)

    for record in records:
        content = Path(record.path).read_text(encoding="utf-8")
        assert "export LEGROOM_WORKSPACE_DIR=/custom/workspace" in content


def test_render_runner_scripts_writes_windows_scripts(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "win32")
    monkeypatch.setattr(
        "legroom.install.supervisors.resolve_legroom_command", lambda: ["legroom.exe"]
    )
    monkeypatch.setattr(
        "legroom.install.supervisors.windows_run_script_path",
        lambda profile: tmp_path / "run-legroom.ps1",
    )
    monkeypatch.setattr(
        "legroom.install.supervisors.windows_run_cmd_path",
        lambda profile: tmp_path / "run-legroom.cmd",
    )
    monkeypatch.setattr(
        "legroom.install.supervisors.windows_ensure_script_path",
        lambda profile: tmp_path / "ensure-legroom.ps1",
    )
    monkeypatch.setattr(
        "legroom.install.supervisors.windows_ensure_cmd_path",
        lambda profile: tmp_path / "ensure-legroom.cmd",
    )
...
def test_start_and_stop_supervisor_use_linux_systemctl(monkeypatch) -> None:
    ...
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr(
        "legroom.install.supervisors.subprocess.run",
    ...
    assert calls == [
        ["systemctl", "--user", "restart", "legroom-default"],
        ["systemctl", "--user", "stop", "legroom-default"],
    ]
...
def test_install_supervisor_linux_service_and_tasks(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "linux")
    run_script = tmp_path / "run-legroom.sh"
    ensure_script = tmp_path / "ensure-legroom.sh"
    monkeypatch.setattr(
        "legroom.install.supervisors.render_runner_scripts",
        lambda manifest: [
            type("Record", (), {"kind": "script", "path": run_script.as_posix()})(),
            type("Record", (), {"kind": "script", "path": ensure_script.as_posix()})(),
        ],
    )
    unit_path = tmp_path / "legroom-default.service"
    monkeypatch.setattr(
        "legroom.install.supervisors._linux_service_unit",
    ...
    monkeypatch.setattr("legroom.install.supervisors.subprocess.run", fake_run)
    ...
    assert ["systemctl", "--user", "enable", "legroom-default"] in [call[0] for call in calls]
    assert service_records[-1].kind == "service-unit"

    cron_path = tmp_path / "legroom-system"
    monkeypatch.setattr(
        "legroom.install.supervisors._linux_task_spec",
...
def test_install_supervisor_darwin_windows_and_unsupported(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("legroom.install.supervisors.os.getuid", lambda: 123, raising=False)

    plist_path = tmp_path / "com.legroom.default.plist"
    monkeypatch.setattr(
        "legroom.install.supervisors._macos_launchd_plist",
        lambda manifest, script, interval=None: (plist_path, f"plist-{interval}"),
    )
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "darwin")
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "darwin")
    service_records = install_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    ...
    monkeypatch.setattr(
        "legroom.install.supervisors.windows_run_cmd_path",
        lambda profile: Path(f"C:\\tmp\\{profile}\\run-legroom.cmd"),
    )
    monkeypatch.setattr(
        "legroom.install.supervisors.windows_ensure_cmd_path",
        lambda profile: Path(f"C:\\tmp\\{profile}\\ensure-legroom.cmd"),
    )
    ...
    assert (
        "sc.exe create legroom-default "
        'binPath= "cmd.exe /c \\"C:\\tmp\\default\\run-legroom.cmd\\"" start= auto'
    ) in calls
    assert [
        "schtasks",
        "/Create",
        "/TN",
        "legroom-default-health",
        "/TR",
        "C:\\tmp\\default\\ensure-legroom.cmd",
        "/SC",
        "MINUTE",
        "/MO",
        "5",
    ...
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "plan9")
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "plan9")
...
def test_install_supervisor_retries_bootstrap_until_launchd_settles(
    ...
    monkeypatch.setattr("legroom.install.supervisors.sys.platform", "darwin")
    monkeypatch.setattr("legroom.install.supervisors.os.getuid", lambda: 123, raising=False)
    monkeypatch.setattr("legroom.install.supervisors.time.sleep", lambda _s: None)
...
def test_install_supervisor_raises_after_bootstrap_keeps_failing(
    ...
    monkeypatch.setattr("legroom.install.supervisors.os.getuid", lambda: 123, raising=False)
    monkeypatch.setattr("legroom.install.supervisors.time.sleep", lambda _s: None)
    monkeypatch.setattr("legroom.install.supervisors._MACOS_BOOTSTRAP_RETRIES", 3)
...
def test_start_and_stop_supervisor_darwin_windows_and_none(monkeypatch) -> None:
    ...
    monkeypatch.setattr("legroom.install.supervisors.os.getuid", lambda: 77, raising=False)
    ...
    assert calls == [
        ["launchctl", "kickstart", "-k", "gui/77/com.legroom.default"],
        ["launchctl", "bootout", "gui/77/com.legroom.default"],
    ]
    ...
    stop_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    assert calls == [
        ["sc.exe", "start", "legroom-default"],
        ["sc.exe", "stop", "legroom-default"],
    ]
...
def test_macos_start_bootstraps_when_job_not_registered(monkeypatch, tmp_path: Path) -> None:
    ...
    plist_path = tmp_path / "Library" / "LaunchAgents" / "com.legroom.default.plist"
    assert calls == [
        ["launchctl", "kickstart", "-k", "gui/77/com.legroom.default"],
...
def test_macos_stop_tolerates_missing_job(monkeypatch) -> None:
    ...
    stop_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    assert calls == [["launchctl", "bootout", "gui/77/com.legroom.default"]]
...
def test_remove_supervisor_linux_service_cron_path_and_missing_crontab(
    ...
    assert ["systemctl", "--user", "disable", "--now", "legroom-default"] in calls
    assert ["systemctl", "--user", "daemon-reload"] in calls

    cron_path = tmp_path / "legroom-task"
...
def test_remove_supervisor_darwin_and_windows(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("legroom.install.supervisors.os.getuid", lambda: 55, raising=False)

    plist_path = tmp_path / "com.legroom.default.plist"
    plist_path.write_text("plist", encoding="utf-8")
    monkeypatch.setattr(
        "legroom.install.supervisors.unix_run_script_path",
        lambda profile: tmp_path / "run-legroom.sh",
    )
    monkeypatch.setattr(
        "legroom.install.supervisors.unix_ensure_script_path",
        lambda profile: tmp_path / "ensure-legroom.sh",
    )
    monkeypatch.setattr(
        "legroom.install.supervisors._macos_launchd_plist",
        lambda manifest, script, interval=None: (plist_path, "plist"),
    ...
    assert calls[0] == ["launchctl", "bootout", "gui/55/com.legroom.default"]
    ...
    assert calls == [
        ["sc.exe", "stop", "legroom-default"],
        ["sc.exe", "delete", "legroom-default"],
        ["schtasks", "/Delete", "/TN", "legroom-default-startup", "/F"],
        ["schtasks", "/Delete", "/TN", "legroom-default-health", "/F"],
    ]
