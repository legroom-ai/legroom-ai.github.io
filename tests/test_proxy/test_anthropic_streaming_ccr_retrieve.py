...
def test_unresolved_ccr_only_still_fails_closed_with_502() -> None:
    """True conversion/handling-error branch: headroom_retrieve remains with no
    accompanying client tool after handling (e.g. the model keeps re-calling it
    up to max rounds). This is not a legal skip — the proxy must fail closed
    with a 502 rather than emit an unresolvable retrieval to the client."""
    config = _make_config()
    # Continuation keeps re-emitting headroom_retrieve alone, so handling exhausts
    # its rounds and returns a response that still contains only CCR calls.
    persistent_ccr_response = _message_response(
        [
            {
                "type": "tool_use",
                "id": "toolu_ccr",
    ...
    with patch("headroom.proxy.server.AnyLLMBackend"):
        ...
        with TestClient(app) as client:
            ...
            continuation_client = _ContinuationClient(persistent_ccr_response)
            proxy.http_client = continuation_client

            async def _fake_retry(method, url, headers, body, stream=False, **kwargs):  # noqa: ANN001
                assert body["stream"] is False
                return httpx.Response(200, json=persistent_ccr_response)
