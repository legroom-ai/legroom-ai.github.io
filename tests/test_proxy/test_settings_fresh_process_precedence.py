...
``legroom.settings_store`` cold and reports what it observes, closing the
...
def workspace(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv(paths.LEGROOM_SETTINGS_PATH_ENV, raising=False)
    return tmp_path
...
def test_env_beats_file_beats_default_in_subprocess(workspace, monkeypatch):
    ...
    script = (
        ...
        "print(json.dumps({'rpm': os.environ.get('LEGROOM_RPM'), "
        "'target_ratio': os.environ.get('LEGROOM_TARGET_RATIO')}))\n"
    )
    env = dict(os.environ)
    env["LEGROOM_WORKSPACE_DIR"] = str(workspace)
    env["LEGROOM_RPM"] = "999"  # explicit export: must win over the file's 20
    env.pop(
        "LEGROOM_TARGET_RATIO", None
