...
- 2.1 optional inbound auth token (LEGROOM_PROXY_TOKEN) on the data plane
- 3.1 response security headers
- 2.4 admin/state-mutating audit log
- 2.2 air-gap master switch (LEGROOM_OFFLINE)
"""
...
from legroom.proxy.audit import is_auditable_path
...
class TestInboundAuthToken:
    ...
    def test_token_set_accepts_custom_header(self):
        app = _make_app(proxy_token="s3cr3t-token")
        with TestClient(app, base_url="http://testserver", client=NONLOOPBACK) as c:
            resp = c.get("/stats", headers={"X-Legroom-Proxy-Token": "s3cr3t-token"})
            assert resp.status_code != 401
...
class TestAdminAuditLog:
    ...
    def test_cache_clear_emits_audit_event(self):
        ...
        assert any("legroom_admin_audit" in m for m in messages)
...
class TestOfflineSwitch:
    def test_is_offline_reads_env(self, monkeypatch):
        monkeypatch.delenv("LEGROOM_OFFLINE", raising=False)
        assert is_offline() is False
        monkeypatch.setenv("LEGROOM_OFFLINE", "1")
        assert is_offline() is True
        monkeypatch.setenv("LEGROOM_OFFLINE", "off")
        assert is_offline() is False
    ...
    def test_offline_disables_update_check(self, monkeypatch):
        from legroom.update_check import is_update_check_enabled
