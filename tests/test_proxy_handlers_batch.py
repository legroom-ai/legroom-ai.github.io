...
from legroom.proxy.handlers import batch as batch_module
...
class DummyBatchHandler(batch_module.BatchHandlerMixin, GeminiHandlerMixin):
    ...
    async def _record_request_outcome(self, outcome) -> None:  # noqa: ANN001
        # Mirror of LegroomProxy._record_request_outcome for the batch
    ...
    def _extract_tags(self, headers: dict) -> dict[str, str]:
        # Mirror of LegroomProxy._extract_tags. Handlers now call this
        # at entry to capture x-legroom-* slicing tags into the outcome.
        return {
    ...
    async def _run_compression_in_executor(self, fn, *, timeout):  # noqa: ANN001, ANN201
        # Mirror of LegroomProxy._run_compression_in_executor: batch handlers
...
def install_batch_support_modules(
    ...
    *,
    ...
    monkeypatch.setitem(sys.modules, "legroom.ccr", SimpleNamespace(CCRToolInjector=FakeInjector))
    monkeypatch.setitem(
        sys.modules,
        "legroom.tokenizers",
...
async def test_handle_batch_create_validates_json_and_required_fields(
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", raise_bad_json)
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", missing_file_payload)
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", missing_endpoint_payload)
...
async def test_handle_batch_create_passthrough_and_download_failure(
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", passthrough_payload)
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", download_missing_payload)
...
async def test_handle_batch_create_handles_empty_upload_failure_and_success(
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", request_payload)
    ...
    assert success_headers["x-legroom-tokens-saved"] == "10"
    assert success_headers["x-legroom-savings-percent"] == "50.0"
    ...
    assert sent_body["metadata"]["legroom_compressed"] == "true"
    assert sent_body["metadata"]["legroom_original_file_id"] == "file-1"
...
async def test_handle_google_batch_create_validates_and_passthroughs(
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", bad_json)
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", no_inline)
...
async def test_handle_google_batch_create_success_and_failure_paths(
    ...
    monkeypatch.setattr("legroom.proxy.helpers._read_request_json", good_payload)
