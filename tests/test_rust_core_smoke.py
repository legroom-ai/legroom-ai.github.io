...
Background — Finding #2 of LEGROOM_PROXY_LOG_FINDINGS_2026_05_03.md.
A customer's production proxy was silently running without the
`legroom._core` PyO3 extension because the Docker image never built it
...
1. The PyO3 module exposes a `hello()` marker function returning
   ``"legroom-core"`` so the deployment smoke test has something stable
...
2. The proxy lifespan refuses to start when ``legroom._core`` is
...
3. An explicit opt-out — ``LEGROOM_REQUIRE_RUST_CORE=false`` — keeps
...
def test_rust_core_imports() -> None:
    """`legroom._core.hello()` returns the documented sentinel.

    The deployment smoke test (`legroom.proxy.server._check_rust_core`)
    ...
    """
    from legroom._core import hello

    assert hello() == "legroom-core"
...
def test_proxy_refuses_to_start_when_rust_core_missing(
    ...
    """When `legroom._core` raises ImportError on import and the opt-out
    ...
    monkeypatch.delenv("LEGROOM_REQUIRE_RUST_CORE", raising=False)

    # Force the import to raise. Patching the symbol on the module is
    # not enough because `_check_rust_core` runs `from legroom._core
    ...
    sys.modules.pop("legroom._core", None)
    ...
    # sys.modules["legroom._core"] to None forces a real ImportError.
    sys.modules["legroom._core"] = None  # type: ignore[assignment]
...
# ──────────────────────────────────────────────────────────────────────────
# 3. Opt-out path: LEGROOM_REQUIRE_RUST_CORE=false → degraded mode + /health.
# ──────────────────────────────────────────────────────────────────────────
def test_proxy_starts_in_degraded_mode_when_opt_out_set(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """With the opt-out env var set, the lifespan must start successfully
    even when `legroom._core` is missing, and `/health` must surface
    ...
    # sys.modules["legroom._core"] = None makes Python treat the module
...
def test_check_rust_core_returns_loaded_when_extension_present(
    ...
    """When `legroom._core` is loadable and `hello()` returns the
