...
from legroom.providers.opencode.config import (
    LEGROOM_OPENCODE_PLUGIN,
    _inject_key_into_json,
    _parse_json_loose,
    append_legroom_plugin,
...
def test_opencode_config_paths_default(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert backup_file == tmp_path / ".config" / "opencode" / "opencode.json.legroom-backup"
...
def test_opencode_config_paths_from_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert backup_file == tmp_path / "custom" / "opencode.json.legroom-backup"
...
def test_opencode_config_paths_default_jsonc(
    ...
    assert backup_file == base_dir / "opencode.jsonc.legroom-backup"
...
def test_snapshot_creates_backup(tmp_path: Path) -> None:
    ...
    backup_file = tmp_path / "opencode.json.legroom-backup"
...
def test_snapshot_skips_if_markers_present(tmp_path: Path) -> None:
    """snapshot skips if the config already contains Legroom markers."""
    config_file = tmp_path / "opencode.json"
    backup_file = tmp_path / "opencode.json.legroom-backup"
    config_file.write_text("// --- Legroom proxy provider ---\n{}")
    snapshot_opencode_config_if_unwrapped(config_file, backup_file)
...
def test_strip_blocks_removes_provider_and_mcp() -> None:
    ...
    content = (
        "// --- Legroom proxy provider ---\n"
        '{"provider": {}}\n'
        "// --- end Legroom proxy provider ---\n"
        "// --- Legroom MCP server ---\n"
        '{"mcp": {}}\n'
...
def test_inject_key_merges_dicts() -> None:
    ...
    data = _inject_key_into_json(data, "provider", {"legroom": {}})
    assert "openai" in data["provider"]
    assert "legroom" in data["provider"]


def test_inject_key_overwrites_non_dict() -> None:
    """_inject_key_into_json overwrites when existing value is not a dict."""
    data = {"model": "gpt-4o"}
    data = _inject_key_into_json(data, "model", "legroom/claude-sonnet-4-6")
    assert data["model"] == "legroom/claude-sonnet-4-6"
...
def test_inject_provider_config_creates_file(
    ...
    # Bare model ids: OpenCode resolves them as "legroom/<id>" (#1657).
    models = config["provider"]["legroom"]["models"]
    assert "claude-sonnet-4-6" in models
    assert all(not model_id.startswith("legroom/") for model_id in models)
...
def test_inject_provider_config_idempotent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
    config = _parse_json_loose(config_file.read_text())
    assert config["provider"]["legroom"]["options"]["baseURL"] == "http://127.0.0.1:9999/v1"
...
def test_parse_json_loose_handles_comments_and_urls() -> None:
    ...
    data = _parse_json_loose(text)
    assert data["baseURL"] == "http://127.0.0.1:8787/v1"
    assert data["name"] == "Legroom // Proxy"
...
def test_strip_blocks_handles_empty_string() -> None:
    """strip_opencode_legroom_blocks returns empty string for empty input."""
    assert strip_opencode_legroom_blocks("") == ""


def test_strip_blocks_handles_whitespace_only() -> None:
    """strip_opencode_legroom_blocks returns empty string for whitespace input."""
    assert strip_opencode_legroom_blocks("  \n  ") == ""


def test_strip_blocks_preserves_non_legroom_jsonc() -> None:
    """strip_opencode_legroom_blocks preserves JSONC comments not from Legroom."""
...
def test_strip_blocks_removes_only_one_of_two_identical_blocks() -> None:
    """strip_opencode_legroom_blocks removes all provider blocks, not just the first."""
    from legroom.providers.opencode.config import _PROVIDER_MARKER_END, _PROVIDER_MARKER_START
...
def test_strip_blocks_handles_only_mcp_markers() -> None:
    """strip_opencode_legroom_blocks also strips MCP markers."""
    from legroom.providers.opencode.config import _MCP_MARKER_END, _MCP_MARKER_START
...
def test_inject_provider_config_preserves_existing_mcp(
    ...
    """inject_opencode_provider_config preserves MCP without adding legroom."""
    _set_test_home(monkeypatch, tmp_path)
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
    ...
    assert "legroom" not in config["mcp"]
...
def test_inject_provider_config_idempotent_with_complex_config(
    ...
    assert "legroom" in config["provider"]
...
def test_append_legroom_plugin_adds_plugin_once() -> None:
    config: dict[str, object] = {"plugin": ["some-plugin"]}

    assert append_legroom_plugin(config) is True
    assert append_legroom_plugin(config) is False

    assert config["plugin"] == ["some-plugin", LEGROOM_OPENCODE_PLUGIN]


def test_append_legroom_plugin_preserves_configured_tuple_entry() -> None:
    config: dict[str, object] = {
        "plugin": [[LEGROOM_OPENCODE_PLUGIN, {"proxyUrl": "http://127.0.0.1:8787"}]]
    }

    assert append_legroom_plugin(config) is False
    assert config["plugin"] == [[LEGROOM_OPENCODE_PLUGIN, {"proxyUrl": "http://127.0.0.1:8787"}]]
...
def test_build_opencode_config_content_without_mcp(
    ...
    from legroom.providers.opencode.runtime import build_opencode_config_content

    plugin = tmp_path / "entry.opencode.js"
    plugin.write_text("export default () => {}", encoding="utf-8")
    monkeypatch.setenv("LEGROOM_OPENCODE_PLUGIN_PATH", str(plugin))

    config = build_opencode_config_content(port=8787, include_mcp=False)
    assert "mcp" not in config
    assert "model" not in config
    # Native providers are pointed at the proxy so traffic routes through Legroom.
    ...
    # The legroom provider exposes explicit models so "legroom/<id>" resolves (#1657).
    assert providers["legroom"]["options"]["baseURL"] == "http://127.0.0.1:8787/v1"
    models = providers["legroom"]["models"]
    assert "claude-sonnet-4-6" in models
    assert all(not model_id.startswith("legroom/") for model_id in models)
...
def test_build_opencode_config_content_skips_plugin_when_unbuilt(
    ...
    monkeypatch.setenv("LEGROOM_OPENCODE_PLUGIN_PATH", str(tmp_path / "missing.js"))
...
def test_build_launch_env_with_project(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    from legroom.providers.opencode.runtime import build_launch_env
    ...
    assert env["LEGROOM_PROJECT"] == "test-proj"
    # Plugin loaded → its proxy target is exported for self-configuration.
    assert env["LEGROOM_PROXY_URL"] == "http://127.0.0.1:8787"
    assert str(plugin) in env["OPENCODE_CONFIG_CONTENT"]
    assert f"plugin={LEGROOM_OPENCODE_PLUGIN}" in display
...
def test_proxy_base_url() -> None:
    from legroom.providers.opencode.runtime import proxy_base_url
...
def test_inject_provider_config_strips_existing_markers(
    ...
    assert "legroom" in first

    inject_opencode_provider_config(port=9001)
    second = config_file.read_text()
    assert "legroom" in second
    assert second.count("legroom") == first.count("legroom")
