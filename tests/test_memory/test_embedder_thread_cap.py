...
``LEGROOM_EMBED_CONCURRENCY x LEGROOM_EMBED_NUM_THREADS``. The ONNX embedder
...
from legroom.memory.adapters import embedders
from legroom.memory.adapters.embedders import (
...
def test_concurrency_reads_positive_int(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("LEGROOM_EMBED_CONCURRENCY", "7")
...
def test_worker_init_sets_blas_env_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("LEGROOM_EMBED_NUM_THREADS", "2")
...
def test_worker_init_does_not_override_operator_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("LEGROOM_EMBED_NUM_THREADS", "1")
...
async def test_cpu_embed_workers_are_thread_capped(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("LEGROOM_EMBED_NUM_THREADS", "1")
    monkeypatch.setenv("LEGROOM_EMBED_CONCURRENCY", "2")
