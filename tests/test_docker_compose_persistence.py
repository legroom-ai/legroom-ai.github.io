...
def test_top_level_compose_pins_legroom_state_to_named_volume() -> None:
    compose = (ROOT / "docker-compose.yml").read_text(encoding="utf-8")

    assert "- legroom_workspace:/home/nonroot/.legroom" in compose
    assert "- HOME=/home/nonroot" in compose
    assert "- LEGROOM_WORKSPACE_DIR=/home/nonroot/.legroom" in compose
    assert "- LEGROOM_CONFIG_DIR=/home/nonroot/.legroom/config" in compose
...
def test_top_level_compose_marks_source_build_version() -> None:
    ...
    assert "LEGROOM_BUILD_VERSION: ${LEGROOM_BUILD_VERSION:-source-build}" in compose
    assert 'ARG LEGROOM_BUILD_VERSION=""' in dockerfile
    ...
    assert "import legroom._version" not in dockerfile
