"""LegroomContribution ratio methods."""

from __future__ import annotations

from legroom.subscription.models import LegroomContribution
...
def test_efficiency_pct_never_exceeds_100_from_cache_reads():
    ...
    forwarded, not tokens Legroom removed. Counting them in the numerator while
    the denominator excludes them let efficiency report impossible values like
    1000%.
    """
    c = LegroomContribution(tokens_submitted=100, tokens_saved_cache_reads=1000)
...
def test_efficiency_pct_is_compression_removal_ratio():
    """Efficiency is compression + CLI filtering over the pre-Legroom input."""
    c = LegroomContribution(
...
def test_efficiency_pct_zero_when_no_input():
    assert LegroomContribution().efficiency_pct() == 0.0
