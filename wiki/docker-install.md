...
```bash
curl -fsSL https://raw.githubusercontent.com/ghaliba3/headroom/main/scripts/install.sh | "$(brew --prefix bash)/bin/bash"
```
...
2. Pulls `ghcr.io/ghaliba3/headroom:latest` by default, or reuses / pulls `HEADROOM_DOCKER_IMAGE` when you set a custom image override.
...
Published releases also push versioned GHCR tags such as `ghcr.io/ghaliba3/headroom:0.5.26`, and those images are built with the same synced package version used for the matching PyPI and npm release.
