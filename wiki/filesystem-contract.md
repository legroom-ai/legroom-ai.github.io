...
Legroom writes configuration, runtime state, logs, and caches to a small
...
(`LEGROOM_SAVINGS_PATH`, `LEGROOM_TOIN_PATH`,
...
- `LEGROOM_WORKSPACE_DIR=/mnt/state` → savings land at
  `/mnt/state/proxy_savings.json` unless `LEGROOM_SAVINGS_PATH` overrides.
- `LEGROOM_SAVINGS_PATH=/custom/savings.json` always wins, even when
  `LEGROOM_WORKSPACE_DIR` is set.
...
`models.json` historically lived at `~/.legroom/models.json` (i.e. in the
...
Python `install` command) sets `LEGROOM_WORKSPACE_DIR` and
`LEGROOM_CONFIG_DIR` inside the container so the proxy resolves state to
the bind-mounted path without any user action.

## Project-scoped `.legroom/` directories

A few code paths deliberately use **project-local** `.legroom/` paths
...
These **do not obey** `LEGROOM_WORKSPACE_DIR`. This is intentional: it
