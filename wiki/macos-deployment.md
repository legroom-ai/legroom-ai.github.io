...
This guide covers deploying the legroom proxy server as a background service on macOS using LaunchAgent. The service will start automatically on login and restart on crash.
...
- legroom-ai installed with proxy support
- Anthropic API key configured

### Installing Legroom with Proxy Support

```bash
# Install the host CLI with proxy support
uv tool install --python 3.13 "legroom-ai[proxy]"

# If your shell cannot find `legroom` after installation
...
current Legroom wheel set. Passing `--python 3.13` keeps the CLI install on a
...
```bash
# Clone or navigate to legroom repository
...
1. Detect your legroom installation
...
Edit `~/Library/LaunchAgents/com.legroom.proxy.plist`:

1. Replace `__LEGROOM_PATH__` with your legroom path:

   ```bash
   command -v legroom
   # Example output: /usr/local/bin/legroom
   ```
...
3. Update shell integration: `export LEGROOM_PORT=9000`

### Log Location

Logs are written to standard macOS locations:

- **Standard output**: `~/Library/Logs/legroom/proxy.log`
- **Error output**: `~/Library/Logs/legroom/proxy-error.log`
...
(`LEGROOM_COMPRESSION_PROVIDER=llmlingua`, `LEGROOM_LLMLINGUA_DEVICE`,
the `legroom-ai[llmlingua]` extra) were retired with the
...
source /path/to/legroom/examples/deployment/macos-launchagent/shell-integration.sh
```
...
launchctl kickstart -k gui/$(id -u)/com.legroom.proxy

# Manual stop/start
launchctl bootout gui/$(id -u)/com.legroom.proxy
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.legroom.proxy.plist
```
...
launchctl disable gui/$(id -u)/com.legroom.proxy

# Re-enable
launchctl enable gui/$(id -u)/com.legroom.proxy
```
...
Should show legroom listening on port 8787.
...
```bash
tail -n 20 ~/Library/Logs/legroom/proxy-error.log
```
...
| `ModuleNotFoundError: No module named 'legroom'` | Install: `uv tool install --python 3.13 "legroom-ai[proxy]"` |
| `command not found: legroom` | Update plist with correct path: `command -v legroom` |
...
- Missing dependencies: `uv tool install --python 3.13 "legroom-ai[proxy]"`
...
```bash
# Should be set to 1
echo $LEGROOM_SHELL_INTEGRATION_LOADED
```
...
```bash
grep -A1 RunAtLoad ~/Library/LaunchAgents/com.legroom.proxy.plist
```
...
```bash
chmod 644 ~/Library/LaunchAgents/com.legroom.proxy.plist
```

**Verify ownership:**

```bash
ls -l ~/Library/LaunchAgents/com.legroom.proxy.plist
```
...
# Remove plist
rm ~/Library/LaunchAgents/com.legroom.proxy.plist

# Remove logs (optional)
rm -rf ~/Library/Logs/legroom
```
...
- [Architecture](ARCHITECTURE.md) - How Legroom works internally
...
cp com.legroom.proxy.plist.template ~/Library/LaunchAgents/com.legroom.proxy-2.plist
# Edit: Change Label to com.legroom.proxy-2, port to 8788
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.legroom.proxy-2.plist
```
...
```xml
<key>LEGROOM_EMBEDDER_RUNTIME</key>
...
**Q: Why LaunchAgent instead of running `legroom proxy` manually?**
...
A: Yes, fully compatible. ML compression (Kompress, opt-in via `legroom-ai[ml]`) auto-detects MPS on Apple Silicon.
